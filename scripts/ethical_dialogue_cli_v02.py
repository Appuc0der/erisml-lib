#!/usr/bin/env python
"""
ethical_dialogue_cli_v02.py

Interactive, YAML-driven DEME ethics dialogue that builds a
DEMEProfileV02 instance and writes it as JSON.

Uses the narrative ethical_dialogue_questions.yaml you already have.
"""
...
from erisml.ethics.profile_v02 import (
    DEMEProfileV02,
    PrinciplismWeights,
...
def intro(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    print(
        ...
        "group, which will become a DEMEProfileV02.\n"
    )
    ...
    # Domain scope list (for your own use; not strictly needed in V02, but handy)
    domain_options = [
        "domestic",
        "clinical",
        "maritime",
...
def ask_risk_appetite() -> RiskAttitudeProfile:
    ...
    # Simple default tolerances; you can refine later.
...
# ---------------------------------------------------------------------------
# Derivation helpers: from dimension scores -> Principlism + NIST weights
# ---------------------------------------------------------------------------
...
def derive_deme_dimensions(norm_scores: Dict[str, float]) -> DEMEDimensionWeights:
    return DEMEDimensionWeights(
        ...
        trust_relationships=0.05,  # neutral default, can be tuned later
    )


def derive_principlism(dim: DEMEDimensionWeights) -> PrinciplismWeights:
    # crude but clear: map DEME dimensions into principlist axes
...
def derive_trustworthiness(dim: DEMEDimensionWeights) -> TrustworthinessWeights:
    # Again, heuristic; refine later as needed.
    valid_reliable = 0.2  # neutral
    safe = dim.safety
    secure_resilient = dim.rule_following_legality * 0.5 + 0.1
    accountable_transparent = dim.rule_following_legality * 0.5 + 0.1
    explainable_interpretable = 0.1  # placeholder; could be its own dialogue item
    ...
    return TrustworthinessWeights(
        ...
        explainable_interpretable=explainable_interpretable / total,
...
def derive_hard_vetoes(flags: Dict[str, bool]) -> HardVetoes:
    return HardVetoes(
        ...
        # the others keep their defaults for now
    )
    ...
    if flags.get("never_needless_harm_to_non_human_life", True):
        pcs.append(
            PatternConstraint(
                ...
                kind=PatternConstraintKind.FORBID_WHEN,
                expression="societal_and_environmental.harms_non_human_life == true "
...
def run_dialogue(config: Dict[str, Any]) -> DEMEProfileV02:
    ident = intro(config)
    ...
    # simple lexical priorities depending on override mode
    if override_mode == OverrideMode.RIGHTS_FIRST:
        lexical = ["rights", "safety", "fairness", "utility"]
    elif override_mode == OverrideMode.CONSEQUENCES_FIRST:
        lexical = ["utility", "safety", "rights", "fairness"]
    else:
        lexical = []
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Interactive DEME ethics dialogue (V02 → DEMEProfileV02 JSON)."
    )
    parser.add_argument(
        "--config",
        "-c",
    ...
    parser.add_argument(
        ...
        "-o",
        type=str,
        default="em_profile_v02.json",
        help="Path to write resulting profile JSON.",
    )
...
def main() -> None:
    ...
    data = deme_profile_v02_to_dict(profile)
    ...
    print()
    print("=" * 72)
    print(f"DEMEProfileV02 written to: {args.output}")
