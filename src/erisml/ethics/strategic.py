...
class StrategicLayer:
    ...
    def solve_nash(self, payoff_tensor: np.ndarray, agents: List[str], actions: List[str]) -> NashResult:
        n_agents = len(agents)
        ...
        print(f"⚙️  Solving Game Matrix: {n_agents} Agents, {n_actions} Actions ({len(profiles)} outcomes)")
        ...
        return NashResult(
            equilibrium_profiles=equilibria,
            is_unique=(len(equilibria) == 1)
        )

    def _is_stable(self, profile: Tuple[int, ...], payoffs: np.ndarray, n_agents: int, n_actions: int) -> bool:
